...
public class PrintManagerActivity extends Activity {
    ...
    private void renderHtmlToBitmap() {
        ...
        // Disable all automatic scaling and force 1:1 pixel mapping
        float density = getResources().getDisplayMetrics().density;
        offscreenWebView.setInitialScale((int) (100 / density));

        offscreenWebView.setWebViewClient(new WebViewClient() {
            @Override
            public void onPageFinished(WebView view, String url) {
                // Wait for CSS and scaling to stabilize
        ...
        int targetWidth = prefs.getInt("printer_width_px", 576);

        if (htmlDataToPrint.toLowerCase().contains("<html")) {
            // It's already a full document, use as is
            finalHtml = htmlDataToPrint;
        } else {
            // Wrap it with standard thermal printer styling
            finalHtml = "<html><head>" +
                    "<meta charset=\"UTF-8\">" +
                    "<style>body { margin: 0; padding: 0; background: white; width: " + targetWidth + "px; font-family: sans-serif; }</style>" +
                    "</head><body>" 
                    + htmlDataToPrint + "</body></html>";
        }
                          
        offscreenWebView.loadDataWithBaseURL(null, finalHtml, "text/html", "UTF-8", null);
    }
    ...
    private void takeSnapshot() {
        ...
        offscreenWebView.measure(
                ...
                View.MeasureSpec.makeMeasureSpec(30000, View.MeasureSpec.AT_MOST)
        );
